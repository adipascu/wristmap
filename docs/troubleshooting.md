...
| "nav: companion not allowed by watchapp" | The watchapp on the watch is an older build without the companion package in its `companionApp` list. | Reinstall the current `wristmap.pbw`. |
| "nav: watchapp not open" | The watchapp is closed and the message was not one that relaunches it. | Open Wristmap on the watch, or wait for the next instruction, which relaunches it. |
| "nav: no watch connected" | The Pebble app has no connected watch. | Check the Pebble app. |
| "Waiting for GPS" on the watch | Location was not granted, or only "while using", so the service started from the background gets no fixes. | Grant location and "location all the time" in Wristmap's setup section. |
...
| Map does not react to swipes | The watch runs an older watchapp, or touch is disabled system-wide on the watch. | Reinstall `wristmap.pbw`. |
...
`last-frame.wmf`. On a debug build they can be pulled with `run-as be.pascu.wristmap`, and the
