...
static void draw_map(GContext *ctx, GRect area) {
  GBitmap *bitmap = map_frame_bitmap();
  if (!bitmap) {
  ...
  GRect dest = GRect(area.origin.x + (area.size.w - bitmap_bounds.size.w) / 2,
                     area.origin.y + (area.size.h - bitmap_bounds.size.h) / 2,
                     bitmap_bounds.size.w, bitmap_bounds.size.h);
  graphics_context_set_compositing_mode(ctx, GCompOpAssign);
  graphics_draw_bitmap_in_rect(ctx, bitmap, dest);
}

static void draw_arrow_view(GContext *ctx, GRect bounds, const NavState *state) {
  int box_side = bounds.size.w - 40;
  if (box_side > bounds.size.h - 90) {
    box_side = bounds.size.h - 90;
  }
  GRect box = GRect(bounds.origin.x + (bounds.size.w - box_side) / 2, bounds.origin.y + 6, box_side, box_side);
  draw_maneuver(ctx, box, state, box_side / ARROW_SIZE);
  draw_text(ctx, state->distance, FONT_KEY_GOTHIC_28_BOLD,
            GRect(bounds.origin.x + 4, box.origin.y + box_side + 2, bounds.size.w - 8, 32), GTextAlignmentCenter);
  const char *street = state->street[0] ? state->street : state->instruction;
  int street_y = box.origin.y + box_side + 34;
  int street_h = bounds.origin.y + bounds.size.h - BOTTOM_BAR_HEIGHT - 2 - street_y;
  draw_text(ctx, street, FONT_KEY_GOTHIC_18, GRect(bounds.origin.x + 4, street_y, bounds.size.w - 8, street_h),
            GTextAlignmentCenter);
}
...
void ui_draw(GContext *ctx, GRect bounds, ViewMode mode) {
  ...
  if (mode == VIEW_ARROW) {
    draw_arrow_view(ctx, bounds, state);
    draw_bottom_bar(ctx, bounds, state);
    return;
  }
  draw_top_bar(ctx, bounds, state);
  draw_map(ctx, ui_map_area(bounds));
  draw_bottom_bar(ctx, bounds, state);
}
